...
class AwemeInfoSpider(scrapy.Spider):
    ...
    def __init__(self):
        ...
        self.api = '/aweme/v1/aweme/favorite/'
    ...
    def parse(self, response):
        ...
        for aweme_list in aweme_lists:
            ...
            item1['uid'] = aweme_list['author_user_id']  # user与aweme关系表uid
            item['aweme_id'] = aweme_list['aweme_id']  # 作品ID
            item1['like_aweme_id'] = aweme_list['aweme_id']  # 关系表喜欢作品ID
            ...
            try:
                item['title'] = re.findall('\w+', aweme_list['title'])  # 背景音乐名称
        ...
        if has_more == 1:
            ...
            url = re.sub("max_cursor=(\d+)", "max_cursor=" + max_cursor,
                         response.meta['url'])
