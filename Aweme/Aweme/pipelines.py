    ...
    def do_insert(self, cursor, item):
        ...
        # 这是Works_comment 表，10个字段 , 对应的是 Comment_Info_Item.py
            ...
            try:
                ...
                sql = "INSERT INTO user_like_aweme_id(id,uid,like_aweme_id) VALUES (NULL ,%s, %s)"
                cursor.execute(sql, params)
            except Exception as error:
                print(error)
        elif isinstance(item, UserPostAwemeItem):
            try:
                params = (
                    item['uid'], item['post_aweme_id']
                )
                sql = "INSERT INTO user_post_aweme_id(id,uid,post_aweme_id) VALUES (NULL ,%s, %s)"
                cursor.execute(sql, params)
            except Exception as error:
                print(error)
